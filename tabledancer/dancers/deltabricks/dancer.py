...
class DeltabricksBackend:
  ...
  def _get_table_struct_info(self, db_name: str, table_name: str) -> Dict[str, Any]:
    struct_info = self.spark.sql(f"SHOW TABLE EXTENDED in {db_name} LIKE '{table_name}'").limit(1).collect()[0].information
    tokens = filter(lambda x: not is_none_or_empty_string(x), struct_info.split('\n'))
    tokens = [str.lower(t) for t in tokens]
    return dict([x.strip() for x in t.split(':', 1)] for t in tokens)

  def _get_table_col_info(self, db_name: str, table_name: str) -> Generator[Tuple[str, str,str]]:
    col_info = self.spark.sql(f"DESCRIBE TABLE {db_name}.{table_name}").where("data_type != null").collect()
...
class DeltabricksDancer():
  ...
  def dance(self, choreograph: Dict[str, Any]):
    ...
    backend_table_spec = DeltabricksTableSpec.from_ddl_info(self.backend.get_ddl(vc_table_spec.database_name,vc_table_spec.table_name))
